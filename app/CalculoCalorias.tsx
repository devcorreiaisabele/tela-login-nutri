...
export default function CalculoCalorias() {
    ...
    useEffect(() => {
        async function carregar() {
            const pesoSalvo   = await AsyncStorage.getItem('usuarioPeso');
            const idadeSalva  = await AsyncStorage.getItem('usuarioIdade');
            const generoSalvo = await AsyncStorage.getItem('usuarioGenero');
            const alturaSalva = await AsyncStorage.getItem('usuarioAltura');
            if (pesoSalvo)   setPeso(parseFloat(pesoSalvo));
            if (idadeSalva)  setIdade(parseInt(idadeSalva));
            if (generoSalvo) setGenero(generoSalvo);
            if (alturaSalva) setAltura(parseInt(alturaSalva));
        }
        carregar();
    }, []);
    ...
    async function registrarAltura() {
        await AsyncStorage.setItem('usuarioAltura', altura.toString());
...
const styles = StyleSheet.create({
    ...
    cardMeta:             { backgroundColor: '#2E7D32', borderRadius: 24, padding: 30, alignItems: 'center', marginBottom: 28, overflow: 'hidden' },
